...
func TestContainerCleanupManager(t *testing.T) {
	t.Run("when there's no cleanup info stored", func(t *testing.T) {
		...
		require.Equal(t, 0, len(cleanupService.cleanupArgs))
	})
	...
	t.Run("when there is a single stale cleanup info", func(t *testing.T) {
		...
		runCleanupManagerForNTicks(t, cleanupManager, 1)
		require.Equal(t, 1, len(cleanupService.cleanupArgs))

		assert.Equal(t, containerID, cleanupService.cleanupArgs[0].id)
		assert.True(t, cleanupInfo == cleanupService.cleanupArgs[0].cleanupInfo)
	})
	...
	t.Run("when there are some stale cleanup infos, and some non-stale ones", func(t *testing.T) {
		...
		runCleanupManagerForNTicks(t, cleanupManager, 1)
		require.Equal(t, 2, len(cleanupService.cleanupArgs))

		for i := 0; i < 2; i++ {
			assert.Equal(t, containerIDs[i], cleanupService.cleanupArgs[i].id)
			assert.True(t, cleanupInfos[i] == cleanupService.cleanupArgs[i].cleanupInfo)
		}
	})
	...
	t.Run("more complicated run, with several ticks", func(t *testing.T) {
		...
		require.Equal(t, 4, len(cleanupService.cleanupArgs))
		for i := 0; i < 4; i++ {
			assert.Equal(t, containerIDs[i], cleanupService.cleanupArgs[i].id)
			assert.True(t, cleanupInfos[i] == cleanupService.cleanupArgs[i].cleanupInfo)
		}
	...
	t.Run("if several cleanup infos are inserted for the same container, it keeps track of the last insertion time and info", func(t *testing.T) {
		...
		require.Equal(t, 1, len(cleanupService.cleanupArgs))
		assert.Equal(t, containerID, cleanupService.cleanupArgs[0].id)
		assert.True(t, newCleanupInfo == cleanupService.cleanupArgs[0].cleanupInfo)
		if unsafe.Sizeof(*originalCleanupInfo) > 0 {
			...
			assert.False(t, originalCleanupInfo == cleanupService.cleanupArgs[0].cleanupInfo)
		}
	...
	for failures := 1; failures <= maxFailures+1; failures++ {
		t.Run(fmt.Sprintf("it retries failed clean ups, for %d failures - up to %d times", failures, maxFailures), func(t *testing.T) {
			defer setCurrentNanoTimestamps(t,
				time.Second,           // time when inserting
				time.Hour+time.Second, // time for all subsequent ticks
			)()
			defer setCleanupManagerDurations(t, time.Millisecond)()

			cleanupErrors := make([]bool, failures)
			for i := 0; i < failures; i++ {
				cleanupErrors[i] = true
			}
			cleanupService := &dummyCleanupService{cleanupErrors: cleanupErrors}
			cleanupManager := newDockerContainerCleanupManager(cleanupService)

			containerID, cleanupInfo := insertIntoCleanupManager(cleanupManager, 1)

			runCleanupManagerForNTicks(t, cleanupManager, failures+2)

			expectedRetryCount := failures + 1
			if expectedRetryCount > maxFailures {
				expectedRetryCount = maxFailures
			}
			assert.Equal(t, expectedRetryCount, len(cleanupService.cleanupArgs))

			for i := 0; i < len(cleanupService.cleanupArgs); i++ {
				assert.Equal(t, containerID, cleanupService.cleanupArgs[i].id)
				assert.True(t, cleanupInfo == cleanupService.cleanupArgs[i].cleanupInfo)
			}
		})
	}
}
...
type dummyCleanupService struct {
	cleanupArgs []containerCleanupArg
	// the i-th call to performContainerCleanup will return a non-empty error array iff i-1 < len(cleanupErrors) && cleanupErrors[i-1]
	cleanupErrors []bool
}

func (cleanupService *dummyCleanupService) performContainerCleanup(containerID string, cleanupInfo *containerCleanupInfo) []error {
	cleanupService.cleanupArgs = append(cleanupService.cleanupArgs, containerCleanupArg{containerID, cleanupInfo})

	if len(cleanupService.cleanupErrors) != 0 {
		errorsOut := cleanupService.cleanupErrors[0]
		cleanupService.cleanupErrors = cleanupService.cleanupErrors[1:]

		if errorsOut {
			return []error{fmt.Errorf("dummy error")}
		}
	}
	...
	for {
		...
		case containerIDs := <-tickChan:
			if tick < n {
				...
				// cleaned up containers past what we expected
